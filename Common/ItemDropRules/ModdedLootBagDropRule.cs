...
namespace androLib.Common
{
	...
	public class ModdedLootBagDropRule : IItemDropRule
	{
		...
		private static List<IItemDropRule> GetModdedBagLoot(int bagType) {
			...
			if (AndroGlobalNPC.AllItemDropsFromNpcs != null) {
				int count = allModdedLootBagDropRules[bagType].Get().Count;
				if (count <= 0) {
					ModdedBagLoot newModdedBagLoot = new();
					if (!AndroGlobalNPC.AllItemDropsFromNpcs.TryGetValue(bagType, out List<(int, float)> npcs)) {
						if (bagType > ItemID.Count) {
							Item item = bagType.CSI();
							$"{AndroLibGameMessages.UnableDetermineNPCDropsBossBag.ToString().Lang(AndroMod.ModName, L_ID1.AndroLibGameMessages)} {item.Name}, {item.ModFullName()}.".LogNT(ChatMessagesIDs.BossBagDropsFailToFind);
						}
					}
					else if (npcs.Count > 0) {
						NPC npc = npcs.First().Item1.CSNPC();
						AndroGlobalNPC.StaticInstance.TryGetLoot(newModdedBagLoot, npc, true);
					}
