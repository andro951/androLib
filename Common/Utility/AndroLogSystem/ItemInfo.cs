...
using static androLib.Common.Utility.LogSystem.Wiki;
...
namespace androLib.Common.Utility.LogSystem
{
    public class ItemInfo {
        ...
        public ItemInfo(AndroModItem androModItem) {
            this.androModItem = androModItem;
            Item = new(androModItem.Type);
        }
        public ItemInfo(Item item) {
            Item = item;
            if (item.ModItem is AndroModItem androModItem)
                this.androModItem = androModItem;
        }
        ...
        public void AddStatistics(WebPage webpage, bool name = true, bool image = true, bool artists = true, bool types = true, bool tooltip = true, bool rarity = true, bool buy = true, bool sell = true, bool research = true) {
            ...
            if (buy && TryGetShopPrice())
                info.Add(new() { "Buy", $"{ShopPrice.GetCoinsPNG()}{(androModItem is ISoldByNPC soldByWitch ? $" ({soldByWitch.SellCondition.ToString().AddSpaces()})" : "")}" });
        ...
        public void AddDrops(WebPage webpage) {
            ...
            if (enemyDrops.ContainsKey(type)) {
                ...
                foreach (int npcNetID in enemyDrops[type].Keys) {
                    ...
                    bool configDrop = androModItem.ConfigOnlyDrop && AndroGlobalNPC.PreHardModeBossTypes.Contains(npcNetID);
        ...
        public static void AddAllDrops(WebPage webpage, Type type = null) {
            ...
            foreach (int itemType in enemyDrops.Keys) {
                ...
                if (type != null && !modItem.InheritsFrom(type))
                    continue;
                ...
                foreach (int npcNetID in enemyDrops[itemType].Keys) {
                    ...
                    bool configDrop = modItem is AndroModItem androModItem && androModItem.ConfigOnlyDrop && AndroGlobalNPC.PreHardModeBossTypes.Contains(npcNetID);
                    string chanceString = $"{(configDrop ? "(" : "")}{chance.PercentString()}{(configDrop ? ")<br/>if config enabled" : "")}";
        ...
        public void AddInfo(WebPage webpage) {
            ...
            if (TryGetShopPrice() && androModItem is ISoldByNPC soldByNPC && soldByNPC.SoldByNPCNetID != null) {
                webpage.NewLine();
                ...
                string sellText = $"The {Item.Name} {soldByNPC.SellCondition.ToNPCSellText(soldByNPC.SoldByNPCNetID, sellPriceString)}";
        ...
        public bool TryGetShopPrice() {
            ...
            if (androModItem is ISoldByNPC soldByWitch && soldByWitch.SellCondition != SellCondition.Never) {
                ShopPrice = (int)((float)Item.type.CSI().value * soldByWitch.SellPriceModifier);
                return true;
            }
        ...
        public static bool TryGetShopPrice(ModItem modItem, out int price) {
            if (modItem is ISoldByNPC soldByWitch && soldByWitch.SellCondition != SellCondition.Never) {
                price = (int)((float)modItem.Type.CSI().value * soldByWitch.SellPriceModifier);
                return true;
            }
        ...
        public Table<string> RecipesCreateItemTable {
            get {
                IEnumerable<IEnumerable<string>> recipesCreateItem = RecipesCreateItem;
                if (!recipesCreateItem.Any())
                    return null;

				return new Table<string>(RecipesCreateItem, label: "Recipes", firstRowHeaders: true, rowspanColumns: true, collapsible: true);
			}
        }
        public Table<string> RecipesUsedInTable {
			get {
				IEnumerable<IEnumerable<string>> recipesUsedIn = RecipesUsedIn;
				if (!recipesUsedIn.Any())
					return null;

				return new Table<string>(RecipesUsedIn, label: "Used in", firstRowHeaders: true, rowspanColumns: true, collapsible: true);
			}
		}
        public Table<string> RecipesReverseRecipesTable {
            get {
				IEnumerable<IEnumerable<string>> recipesReverseRecipes = RecipesReverseRecipes;
				if (!recipesReverseRecipes.Any())
					return null;

				return new Table<string>(RecipesReverseRecipes, label: "Used in (Reverse Crafting Recipes)", firstRowHeaders: true, rowspanColumns: true, collapsible: true);
			}
        }
        private IEnumerable<IEnumerable<string>> GetRecipes(bool createItem = false, bool usedIn = false, bool reverseRecipes = false) {
            ...
            if (!recipesAdded)
                $"No recipe found for {Item.S()}".LogSimple();
        ...
        protected int TryCondenseRecipeThreshold => 5;
		private bool TryAddRecipeData(List<List<string>> myRecipes, Dictionary<int, List<RecipeData>> allRecipeData, bool includeReverse = false, bool usedIn = false) {
			...
			//if (allRecipeData[Item.type].Count >= TryCondenseRecipeThreshold) {
				foreach (RecipeData recipeData in allRecipeData[Item.type]) {
				    PreCondenseModifyRecipeData(recipeData);
				    bool added = false;
            ...
            if (!TryAddRecipeData(myRecipes, myRecipeData, includeReverse, usedIn)) {
                foreach (RecipeData recipeData in myRecipeData) {
					...
					bool reverseRecipe = recipeData.IsReverseRecipe(Item.type) || createItemCountLarger;
        ...
        protected virtual void PreCondenseModifyRecipeData(RecipeData recipeData) {}
        protected virtual bool TryAddRecipeData(List<List<string>> myRecipes, List<RecipeData> myRecipeData, bool includeReverse, bool usedIn) => false;
